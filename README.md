...
- **向第二面注入误差偏置**：外部系统将计算得到的误差信号（同样为 -1 到 1 的整面强度图）一次性注入第二面所有神经元，直接叠加到其膜电位上。正值起兴奋作用，负值起抑制作用。
...
1. 从第二面读取当前的预测输出膜电位；
2. 获取输入，将输入和预测逐位置相减（真实传入值 - 预测值 = 误差值），得到误差图（值域仍在 -1~1 内，图仅代表了一种二维结构，不代表是视觉信号）；
3. 将该误差图一次性注入第二面；
4. 将当前时刻的输入信号注入第一面；
5. 进入下一轮循环。
